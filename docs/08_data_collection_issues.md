...
- 대상: aboutpet.co.kr 강아지/고양이 전체 카테고리 (소분류 98개, 상품 약 18,000개)
- Playwright를 사용하는 이유: AJAX 기반 동적 렌더링 대응 및 세션/쿠키 자동 유지
- 상세 크롤링 스펙: `docs/06_aboutpet_crawling_spec.md` 참고

**서브 이슈**

- [ ] 상품 목록·상세·평점 크롤링 구현
  - `getScateGoodsList`, `getGoodsDetail`, `getGoodsCommentScore` 호출
  - 소분류 98개 순회, `goodsId` 기준 중복 제거
- [ ] 리뷰 크롤링 구현
  - `getGoodsEntireCommentList` 전 페이지 수집
  - `purchase_label` (first/repeat): 조건부 노출 필드. **샘플 크롤링으로 노출 비율 먼저 측정 후** 추천 신호 활용 여부 결정
  - 펫 프로필 (이름·성별·나이·체중·품종): 조건부. XPath 기준 파싱 로직은 **실제 DOM 확인 후 확정**
  - `review_info`: 조건부. 항목 구성이 카테고리마다 상이하므로 **샘플 수집으로 전체 항목 파악 후 스키마 확정**
- [ ] 크롤링 공통 설정
  - 요청 간 딜레이 정책
  - 에러 핸들링 및 재시도 로직
...
- ERD 상세: `docs/03_requirements_spec.md` 섹션 3 참고
